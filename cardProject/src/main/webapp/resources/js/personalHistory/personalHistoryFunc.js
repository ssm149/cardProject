...
var personalHistoryGetData = function(){
	...
	$(".user-info-table1, .user-info-table2").find("input:not(input[name='file']), select").each(function(){ // 유동 테이블 두곳 루프 돌면서 데이터 획득
		...
		if(nowLoopId=="userEmail"){
			...
			//if(emailDomain==null){
			if(emailDomain==null || emailDomain == '직접입력'){
				emailDomain="";
				nowLoopValue = nowLoopValue + $("#emailDomain1").val();
			}
...
var flexibleTableGetData = function(){
	...
	$(".flexibleTable").find("tbody").find("tr").each(function(i){ // 유동 테이블 반복하며 데이터 획득
		...
		if($('#emailDomain').val() == '직접입력'){
			//$('#emailDomain').val($('#emailDomain1').val());
...
var ajaxRequestRegisterList = function(){
	...
	if(userMarry==undefined){
		console.log(userMarry=="undefined");
		userMarry=null;
	}
...
var getRegisterData = function(userIdx){
			...
			$.ajax({
				...
				success:function(data){
					...
					if(imgName==null){
						...
						$("#thumbNail").attr("src","http://localhost:8084/card/"+imgName);
						/*$("#thumbNail").attr("src","http://localhost:8084/img/"+imgName);*/
					}
				}
			});
	...
	for (var key in fixedData) {
		...
		//console.log("aaaa key 1111====> " + key);
		//console.log("aaaa val 2222====> " + val);
		if(key == 'USER_DOMAIN') {
			if(val != '@naver.com' 
				&& val != '@hanmail.net'
				&& val != '@daum.net') {
				//console.log("333333333333333344");
				$("#"+(eval("convertData."+key)+"")).val('직접입력');
				$("#emailDomain1").val(val);
	...
	$(".flexibleTable").each(function(){
      ...
      if (tbName == "hobby") {
         //form 추가
         var hobby_tbody = $('.hobby-table > tbody tr').length;
         if(hobby_tbody > 0){
            $loopTbody.find("tr").remove();
            for(var i = 0; i < nowFlexibleDataLen; i++){
               $loopTbody.append("<tr>" + trText + "</tr>");
            }
         } else if (hobby_tbody == 0) {
            $loopTbody.append("<tr>" +
